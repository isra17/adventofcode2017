...
fn parse_input(sheet: &str) -> Vec<Vec<u32>> {
    sheet.lines()
        .map(|l| {
            l.split_whitespace()
        ...
        .filter(|nums: &Vec<u32>| nums.len() > 0)
        .collect()
}

fn checksum1(sheet: &str) -> u32 {
    parse_input(sheet)
        .into_iter()
        .map(|nums| nums.iter().max().unwrap() - nums.iter().min().unwrap())
        .sum()
}

fn checksum2(sheet: &str) -> u32 {
    parse_input(sheet)
        .into_iter()
        .map(|nums| {
            nums.into_iter()
                .combinations(2)
                .map(|ref t| (max(t[0], t[1]), min(t[0], t[1])))
                .find(|&(a, b)| a % b == 0)
                .map(|(a, b)| a / b)
                .unwrap()
        })
...
fn main() {
    ...
    println!("Part 1: {}", checksum1(input));
    println!("Part 2: {}", checksum2(input));
}
